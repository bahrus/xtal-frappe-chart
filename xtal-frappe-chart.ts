...
<div part=chart-container></div>
`;

export class XtalFrappeChartCore extends HTMLElement implements XtalFrappeChartActions{

    #chart: Chart;
    createChart(self: this){
        const {data, chartTitle, height, colors, type, domCache, toolTipOptions, isNavigable, chartContainerParts} = self;
        ...
        setTimeout(() =>{
            self.#chart = new Chart(chartContainerParts[0], chartOptions);
            setTimeout(() => {
...
const ce = new tm.CE<XtalFrappeChartProps & TemplMgmtBase, XtalFrappeChartActions & INotifyMixin, INotifyPropInfo>({
    config:{
        ...
        actions:{
            ...
            createChart:{
                ...
                actIfKeyIn: ['chartTitle', 'height', 'colors', 'type', 'toolTipOptions', 'isNavigable']
            },
